...
如果只是同步了新的上游版本，自定义版本号会保持不变，例如可以从 `v1.7.30-wx-1.1` 延续到 `v1.7.31-wx-1.1`。
如果 `master` 包含新的前端自定义功能、优化或修复，工作流会自动递增自定义版本号，例如 `v1.7.30-wx-1.0`、`v1.7.30-wx-1.1`、`v1.7.31-wx-1.2`。
...
如果需要人为提升自定义大版本，可以直接从 `master` 打出目标标签，例如 `v1.7.31-wx-2.0`。
