...
- Pushing `master` triggers `.github/workflows/release.yml` to publish the next formal fork release (for example `v1.7.30-wx-1.1`) and upload `dist/management.html`.
- Tagging `vX.Y.Z` still publishes a formal release and uploads the same `dist/management.html` asset.
