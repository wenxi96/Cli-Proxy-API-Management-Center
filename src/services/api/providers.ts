...
export function patchLatestOpenAIProviderRecord(
  latestItems: unknown[],
  name: string,
  index: number,
  patch: Record<string, unknown>
): unknown[] {
  const current = latestItems[index];
  if (!isRecord(current) || !matchesOpenAIProvider(current, name)) {
    throw new Error('Provider configuration changed; refresh and try again.');
  }
  return latestItems.map((item, currentIndex) =>
    currentIndex === index ? { ...current, ...patch } : item
  );
}

export function deleteLatestOpenAIProviderRecord(
  latestItems: unknown[],
  name: string,
  index: number
): unknown[] {
  ...
  return latestItems.filter((_, currentIndex) => currentIndex !== index);
}
...
export const providersApi = {
  ...
  updateOpenAIProviderDisabled: (name: string, index: number, disabled: boolean) =>
    mutateLatestProviderList('openai-compatibility', (latestItems) =>
      patchLatestOpenAIProviderRecord(latestItems, name, index, { disabled })
    ),

  deleteOpenAIProvider: (name: string, index: number) =>
    mutateLatestProviderList('openai-compatibility', (latestItems) =>
      deleteLatestOpenAIProviderRecord(latestItems, name, index)
    ),
