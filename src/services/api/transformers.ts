...
const normalizeScopedPoolProviderConfig = (
  ...
  if (quotaThresholdPercent !== undefined) result.quotaThresholdPercent = quotaThresholdPercent;
  ...
  if (penaltyWindowSeconds !== undefined) result.penaltyWindowSeconds = penaltyWindowSeconds;

  const quotaSnapshotTTLSeconds = normalizeNumericField(
    payload['quota-snapshot-ttl-seconds'] ?? payload.quotaSnapshotTTLSeconds
  );
  if (quotaSnapshotTTLSeconds !== undefined) result.quotaSnapshotTTLSeconds = quotaSnapshotTTLSeconds;

  const idleLogThrottleSeconds = normalizeNumericField(
    payload['idle-log-throttle-seconds'] ?? payload.idleLogThrottleSeconds
  );
  if (idleLogThrottleSeconds !== undefined) result.idleLogThrottleSeconds = idleLogThrottleSeconds;
...
export const normalizeRoutingScopedPoolConfig = (
  ...
  if (isRecord(payload.providers)) {
    ...
    if (Object.keys(providers).length) result.providers = providers;
  }
