...
export function AuthFileCard(props: AuthFileCardProps) {
  ...
  const resolveBatchLabel = (kind: 'classification' | 'bucket' | 'reason', value: string) => {
    const key = `auth_files.batch_check_${kind}_${value}`;
    const translated = t(key);
    return translated === key ? humanizeToken(value) : translated;
  };
  const batchClassificationLabel = batchCheckResult
    ? resolveBatchLabel('classification', batchCheckResult.classification)
    : '';
  const batchBucketLabel = batchCheckResult
    ? resolveBatchLabel('bucket', batchCheckResult.bucket)
    : '';
  const skippedReasonLabel = skippedReason ? resolveBatchLabel('reason', skippedReason) : '';
  const batchBadgeClass =
    batchCheckResult?.classification === 'ok'
      ? legacyStyles.batchCheckBadgeSuccess
      : batchCheckResult?.classification === 'no_quota'
        ? legacyStyles.batchCheckBadgeWarning
        : batchCheckResult
  ...
  return (
    <article className={cardClasses} style={cardStyle}>
      ...
      {showPoolStatus ? (
        <div className={legacyStyles.batchCheckBadgeRow}>
          {poolStateLabel ? (
          ...
          {poolReasonLabel ? (
            <span className={`${legacyStyles.batchCheckBadge} ${legacyStyles.batchCheckBadgeWarning}`}>
              {poolReasonLabel}
            </span>
          ) : null}
      ...
      {batchCheckResult || skippedReasonLabel ? (
        <QuotaCardShell
          ...
          badges={
            batchCheckResult ? (
              <>
                <span className={`${legacyStyles.batchCheckBadge} ${batchBadgeClass}`}>
                  {batchClassificationLabel}
                </span>
                <span
                  className={`${legacyStyles.batchCheckBadge} ${legacyStyles.batchCheckBadgeOutline}`}
                >
                  {batchBucketLabel}
                </span>
              </>
          ...
          meta={
            batchCheckResult ? (
              <span>
                {t('auth_files.batch_check_checked_at')}: {formatDateTime(batchCheckResult.checked_at)}
              </span>
            ) : undefined
          }
        >
          {batchCheckResult ? (
            <>
              ...
              {batchCheckResult.error_message ? (
                <div className={legacyStyles.batchCheckInlineMessage}>{batchCheckResult.error_message}</div>
              ) : null}
            </>
          ) : (
            <div className={legacyStyles.batchCheckInlineMessage}>
