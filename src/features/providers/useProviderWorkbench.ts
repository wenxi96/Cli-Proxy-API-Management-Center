...
export const buildProviderKeyConfig = (
  ...
  const next: ProviderKeyConfig = {
    apiKey: input.apiKeyEdited ? input.apiKey.trim() : (existing?.apiKey ?? ''),
...
export const buildOpenAIConfig = (
  ...
  const apiKeyEntries =
    input.apiKeyEntries
      ?.map((entry) => {
        return {
          apiKey: entry.apiKeyEdited ? entry.apiKey.trim() : (entry.existingApiKey?.trim() ?? ''),
...
const toggleSponsorConfig = async (raw: SponsorProviderRaw, disabled: boolean) => {
  ...
  for (const item of raw.openai) {
    await providersApi.updateOpenAIProviderDisabled(item.config.name, item.index, disabled);
  }
};
    ...
    async (resource: ProviderResource) => {
      ...
      try {
        ...
        if (sel.brand === 'gemini') {
          ...
          await providersApi.deleteOpenAIProvider(sel.name, sel.index);
    ...
    async (resource: ProviderResource, disabled: boolean) => {
      ...
      try {
        ...
        if (brand === 'gemini' && selector.brand === 'gemini') {
          ...
          await providersApi.updateOpenAIProviderDisabled(selector.name, selector.index, disabled);
        } else if (
          brand === 'apikeyFun' ||
          brand === 'code0' ||
          brand === 'fennoAI' ||
