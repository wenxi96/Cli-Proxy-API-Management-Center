    ...
    try {
      ...
      const newValues: VisualConfigValues = {
        ...
        quotaAutoDisableAuthFileQuotaThresholdPercent: String(
          quotaExceeded?.['auto-disable-auth-file-quota-threshold-percent'] ??
            quotaExceeded?.autoDisableAuthFileQuotaThresholdPercent ??
            0
        ),
        ...
        routingScopedPoolDefaultsQuotaThresholdPercent: parseScopedPoolNumber(
          asRecord(scopedPool?.defaults)?.['quota-threshold-percent'] ??
            asRecord(scopedPool?.defaults)?.quotaThresholdPercent
        ),
        routingScopedPoolDefaultsConsecutiveErrorThreshold: parseScopedPoolNumber(
          asRecord(scopedPool?.defaults)?.['consecutive-error-threshold'] ??
            asRecord(scopedPool?.defaults)?.consecutiveErrorThreshold
        ),
        routingScopedPoolDefaultsPenaltyWindowSeconds: parseScopedPoolNumber(
          asRecord(scopedPool?.defaults)?.['penalty-window-seconds'] ??
            asRecord(scopedPool?.defaults)?.penaltyWindowSeconds
        ),
        routingScopedPoolDefaultsQuotaSnapshotTTLSeconds: parseScopedPoolNumber(
          asRecord(scopedPool?.defaults)?.['quota-snapshot-ttl-seconds'] ??
            asRecord(scopedPool?.defaults)?.quotaSnapshotTTLSeconds
        ),
        routingScopedPoolDefaultsIdleLogThrottleSeconds: parseScopedPoolNumber(
          asRecord(scopedPool?.defaults)?.['idle-log-throttle-seconds'] ??
            asRecord(scopedPool?.defaults)?.idleLogThrottleSeconds
        ),
    ...
    (currentYaml: string): string => {
      try {
        ...
        const quotaDirty =
          ...
          dirtyFields.has('quotaAutoDisableAuthFileOnLowQuota') ||
        ...
        if (quotaDirty) {
          ...
          if (dirtyFields.has('quotaAutoDisableAuthFileOnLowQuota')) {
            setBooleanInDoc(
              doc,
          ...
          if (dirtyFields.has('quotaAutoDisableAuthFileQuotaThresholdPercent')) {
            setIntFromStringInDoc(
              doc,
        ...
        const scopedPoolDefaultsDirty =
          dirtyFields.has('routingScopedPoolDefaultsLimit') ||
          dirtyFields.has('routingScopedPoolDefaultsQuotaThresholdPercent') ||
          dirtyFields.has('routingScopedPoolDefaultsConsecutiveErrorThreshold') ||
          dirtyFields.has('routingScopedPoolDefaultsPenaltyWindowSeconds') ||
          dirtyFields.has('routingScopedPoolDefaultsQuotaSnapshotTTLSeconds') ||
          dirtyFields.has('routingScopedPoolDefaultsIdleLogThrottleSeconds');
        const scopedPoolDirty =
          dirtyFields.has('routingScopedPoolEnabled') ||
          dirtyFields.has('routingScopedPoolProviders') ||
          scopedPoolDefaultsDirty;
        const routingDirty =
        ...
        if (routingDirty) {
          ...
          if (scopedPoolDirty) {
            ...
            if (dirtyFields.has('routingScopedPoolEnabled')) {
              setBooleanInDoc(
                doc,
                ['routing', 'scoped-pool', 'enabled'],
